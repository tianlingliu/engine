...
public abstract class AbstractDataSourceConfig {
    protected static final String CONFIG_LOCATION = "classpath:mybatis-config.xml";

    protected DataSourceTransactionManager createTransactionManager(final DataSource dataSource) {
        return new DataSourceTransactionManager(dataSource);
    }

    protected SqlSessionFactory createSqlSessionFactory(final DataSource dataSource,
        final String mapperLocation)
        throws Exception {
        return this.createSqlSessionFactoryBean(dataSource, mapperLocation)
            .getObject();
    }
